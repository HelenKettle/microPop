...
#' @param tradeOffParams (vector of two strings) - parameters to trade off against each other
#' @param numPaths Named vector. Number of paths for each microbial group
#' @param numStrains Scalar. Number of strains per group
#' @param Pmats List containing lists and matrices: [[param]][[strainName]][path,rname]
...
#' 
applyTraitTradeOffs=function(microbeNames,tradeOffParams,numPaths,numStrains,Pmats,resourceNames){
 
  #sort par 1 in the direction of bad to good and par2 in the direction of good to bad
  ...
  for (par in tradeOffParams){
    ...
    #larger yield means more efficient growth on resources
    #large maxGrowthRate means faster growth
    #so direction=increasing
  ...
  for (gname in microbeNames){
    for (path in 1:numPaths[gname]){
      for (rname in resourceNames){
        ...
        if (sum(is.finite(mat[1,]))==numStrains & sum(is.finite(mat[2,]))==numStrains){
