...
derivsDefault=function(t,y,parms){
  ...
  dX=NA*X; names(dX)=parms$allStrainNames
  dX.growth=NA*X; names(dX.growth)=parms$allStrainNames
  dR=NA*R; names(dR)=parms$resourceNames

  dR.g=array(0,dim=c(length(parms$microbeNames),numR),dimnames=list(parms$microbeNames,parms$resourceNames))
  
  for (gname in parms$microbeNames){ #group loop
    ...
    dR.s=matrix(0,ncol=numR,nrow=parms$numStrains,dimnames=list(strainNames,parms$resourceNames))

    for (str.name in strainNames){ #strain loop
      ...
      growthLim=matrix(NA,ncol=numR,nrow=parms$numPaths[gname],dimnames=list(pathNames,parms$resourceNames))
      uptake.p=matrix(0,ncol=numR,nrow=parms$numPaths[gname],dimnames=list(pathNames,parms$resourceNames))
      production.p=matrix(0,ncol=numR,nrow=parms$numPaths[gname],dimnames=list(pathNames,parms$resourceNames))
      
      #find pH limitation on growth for given strain (not path or resource dependent) 
      if (parms$pHLimit){
        pHlim=parms$pHLimFunc(str.name,gname,parms$pHFunc(t,parms),parms)
        #print(pHlim)
      }else{
        pHlim=1}


      for (path in pathNames){ #path loop

          #compute extra growth lim function (default value is 1 ie no limitation)
          extraGrowthLim=parms$extraGrowthLimFunc(str.name,gname,path,c(X,R),c(parms$allStrainNames,parms$resourceNames),t,parms)

          #print(path)
        resnames=colnames(Rtype[gname,,path,drop=FALSE])
          
        subst=resnames[Rtype[gname,,path]=='S' | Rtype[gname,,path]=='Sm']   #substitutable resources
        ...
        for (rname in all.substrates){
           growthLim[path,rname]=parms$growthLimFunc(str.name,gname,path,rname,R,rtype.sub,halfsat.sub,y)
          if (is.na(growthLim[path,rname])){stop(paste('MICROPOP ERROR: growthLimFunc is returning NA for ',str.name,' growing on ',rname,'. (FYI halfSat value is ',halfSat[[str.name]][path,rname],')',sep=''))}
        }
        ...
        growthRate.p[path]=X[str.name]*parms$combineGrowthLimFunc(str.name,gname,path,subst,ess,boost,bio.sub,maxGrowthRate.g.sub,growthLim.sub,parms$keyRes[[gname]][path],parms$nonBoostFrac[gname,,path])
        
        #compute uptake of each resource on this path--------------------
        for (rname in c(all.substrates)){
           uptake.p[path,rname]=X[str.name]*parms$uptakeFunc(str.name,gname,path,rname,parms$keyRes[[gname]][path],subst,ess,boost,maxGrowthRate.g.sub,growthLim.sub,yield.sub,parms$nonBoostFrac[gname,,path],stoichiom.sub,parms)
        }

          #compute uptake of water if it is a resource on this path
          if (any(rtype.sub=='Sw')){
              uptake.p[path,rtype.sub=='Sw']=parms$Pmats$waterRatio[gname,path]*sum(uptake.p[path,],na.rm=TRUE)
          }

        #compute production on this path (metabolites only - not biomass)-----------------------
        for (rname in products){
          production.p[path,rname]=parms$productionFunc(str.name,gname,path,rname,all.substrates,parms$keyRes[[gname]][path],stoichiom.sub,products,bio.products,uptake.p[path,],growthRate.p[path],yield.sub,parms,water)
         }
      ...
      path.frac=parms$combinePathsFunc(str.name,gname,growthRate.p,parms$numPaths[gname],pathNames)
      ...
      if (parms$checkMassConv){
          parms$massBalanceFunc(uptake.p,production.p,growthRate.p,parms$balanceTol,str.name)}
      
      for (rname in parms$resourceNames){
          dR.s[str.name,rname]=sum(production.p[,rname]-uptake.p[,rname],na.rm=T)
      }
    ...
    for (rname in parms$resourceNames){
        ...
        resRate.in=parms$entryRateFunc(rname,R[rname],y,t,parms$Smats$inflowRate,parms)
        if (is.na(resRate.in)){stop(paste('MICROPOP ERROR: entry rate of',rname,'is not defined at time',t))}

        resRate.out=parms$removalRateFunc(rname,R[rname],y,t,washOut,parms)
        if (is.na(resRate.out)){stop(paste('MICROPOP ERROR: removal rate of',rname,'is not defined at time',t))}
 
      dR[rname]=sum(dR.g[,rname],na.rm=TRUE)+resRate.in-resRate.out+dR.m
    }
  ...
  for (str.name in parms$allStrainNames){
      ...
      XRate.in=parms$entryRateFunc(str.name,X[str.name],y,t,parms$Smats$inflowRate,parms)
    
      if (is.na(XRate.in)){stop(paste('MICROPOP ERROR: entry rate of',str.name,'is not defined at time',t))}
      XRate.out=parms$removalRateFunc(getGroupName(str.name,parms$microbeNames),X[str.name],y,t,washOut,parms)
      if (is.na(XRate.out)){stop(paste('MICROPOP ERROR: removal rate of',str.name,'is not defined at time',t))}
